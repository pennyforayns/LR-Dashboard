...
STATUS_FIELDS = [
    ...
    # litter_pct is Whisker's server-side litterLevelPercentage (saturates at
    # 90% and rarely moves). litter_pct_calc is derived locally from the raw
    # ToF distance sensor (pylitterbot litter_level_calculated, 10% steps, can
    # exceed 100% when overfilled) and actually tracks usage; blank while a
    # clean cycle is running. See README.
    "litter_pct", "litter_pct_calc", "litter_state", "waste_drawer_pct", "drawer_full",
...
def _migrate_header(path: Path, fields: "list[str]") -> None:
    """If an existing CSV's header doesn't match `fields` (e.g. a column was
    added to the schema mid-month), rewrite the file in place with the new
    header, filling missing columns with '' for the old rows."""
    if not path.exists():
        return
    with path.open("r", newline="", encoding="utf-8") as f:
        reader = csv.reader(f)
...
CAT_PROFILES = [
    ("Clove", {"sex": "female", "birth": "2020-03-12", "note": "Fixed · indoor"}),
    ("Opal",  {"sex": "female", "birth": "2023-05-23", "note": "Fixed · indoor"}),
]
ROBOT_ORDER = ["The Shitter"]
ROBOT_ALIASES = {"The Shitter": ""}
ROBOT_NIGHTLIGHT = {"The Shitter": "Auto"}
DAILY_KEY = {"The Shitter": "daily_lr1"}


def _classify_cat(weight: float) -> str:
    """Identify the cat from a recorded weight (Clove ≈12, Opal ≈14)."""
    if weight < 13.0:
        return "Clove"
    return "Opal"
...
def _latest_status(log_dir: str):
    ...
    litter_pct_calc is carried forward from the most recent row that has a
    non-blank value, since snapshots taken mid clean-cycle log it blank
    (the ToF sensor can't see the litter bed while the globe is rotated)."""
    latest = {}
    latest_calc = {}
    for fn in glob.glob(os.path.join(log_dir, "litter_robot_logs_*.csv")):
        with open(fn, newline="", encoding="utf-8") as f:
            for r in csv.DictReader(f):
                ...
                if not rb:
                    continue
                if rb not in latest or r["snapshot_time"] > latest[rb]["snapshot_time"]:
                    latest[rb] = r
                calc = (r.get("litter_pct_calc") or "").strip()
                if calc and (rb not in latest_calc or r["snapshot_time"] > latest_calc[rb][0]):
                    latest_calc[rb] = (r["snapshot_time"], calc)
    for rb, row in latest.items():
        if not (row.get("litter_pct_calc") or "").strip() and rb in latest_calc:
            row["litter_pct_calc"] = latest_calc[rb][1]
    return latest


def _litter_history(log_dir: str):
    """Per robot, per day (YYYY-MM-DD): the last non-blank litter_pct_calc
    snapshot of that day, across all monthly status logs."""
    ...
    for fn in glob.glob(os.path.join(log_dir, "litter_robot_logs_*.csv")):
        with open(fn, newline="", encoding="utf-8") as f:
            for r in csv.DictReader(f):
                rb = r.get("robot")
                calc = (r.get("litter_pct_calc") or "").strip()
                ts = r.get("snapshot_time") or ""
                if not rb or not calc or len(ts) < 10:
                    continue
                try:
                    val = int(float(calc))
                except ValueError:
                    continue
                day = ts[:10]
                cur = hist.setdefault(rb, {}).get(day)
                if cur is None or ts > cur[0]:
                    hist[rb][day] = (ts, val)
    return {rb: {d: v[1] for d, v in days.items()} for rb, days in hist.items()}
...
def generate_dashboard(log_dir: str):
    ...
    daily_lr1, daily_lr2 = series("The Shitter"), []

    # Litter level history (sensor-derived litter_pct_calc), one point per day
    # on the same continuous date axis as the daily cycle series. Days without
    # a snapshot carry the previous value forward (rows are only logged on
    # change, so "no row" means "unchanged"); days before the first reading
    # are null.
    lhist = _litter_history(log_dir)
    def litter_series(rb):
        out, last = [], None
        if not cycle_dates:
            return out
        by_day = lhist.get(rb, {})
        ...
        while d <= cycle_dates[-1]:
            ...
            if key in by_day:
                last = by_day[key]
            out.append({"date": key, "pct": last})
            d = d + timedelta(days=1)
        return out

    litter_lr1, litter_lr2 = litter_series("The Shitter"), []
    ...
    for rb in ROBOT_ORDER:
        ...
        try:
            ...
            pass
        try:
            litter_calc = int(float(s["litter_pct_calc"])) if s.get("litter_pct_calc") else None
        except (ValueError, TypeError):
            litter_calc = None
        robots.append({
            "name": rb, "alias": ROBOT_ALIASES.get(rb, ""),
            "litter": int(float(s.get("litter_pct") or 0)),
            "litter_calc": litter_calc,
    ...
    data_json = json.dumps({"daily_lr1": daily_lr1, "daily_lr2": daily_lr2,
                            "litter_lr1": litter_lr1, "litter_lr2": litter_lr2,
                            "visits": visits})

    def robot_js(r):
        litter_key = r["daily_key"].replace("daily_", "litter_")
        tpl = ("{name:%s,alias:%s,litter:%d,litterCalc:%s,drawer:%d,online:%s,drawerFull:%s,"
               "cycles:%s,nightlight:%s,update:false,daily:DATA.%s,hist:DATA.%s}")
        return tpl % (
            json.dumps(r["name"]), json.dumps(r["alias"]), r["litter"],
            json.dumps(r["litter_calc"]), r["drawer"],
            "true" if r["online"] else "false",
            "true" if r["drawerFull"] else "false",
            json.dumps(r["cycles"]), json.dumps(r["nightlight"]), r["daily_key"], litter_key)
...
async def main() -> None:
    ...
    try:
        ...
        for robot in account.robots:
            ...
            # ── Levels & Cycles ──────────────────────────────────────────────
            sub("Levels & Cycles")
            # litter_level is Whisker's server-side litterLevelPercentage —
            # it saturates at 90% and lags. litter_level_calculated is derived
            # from the raw ToF distance sensor and actually tracks usage.
            try:
                litter_calc = robot.litter_level_calculated
            except Exception:
                litter_calc = None
            # While the globe is rotated (clean cycle) the ToF sensor can't
            # see the litter bed and reads ~empty. pylitterbot smooths this
            # over within a long-lived process, but every run of this script
            # is a fresh process, so mid-cycle it returns a bogus 0%. Record
            # blank instead; the dashboard carries the last good value forward.
            try:
                raw = getattr(robot, "_data", {}) or {}
                if raw.get("robotStatus") == "ROBOT_CLEAN" or "litterLevel" not in raw:
                    litter_calc = None
            except Exception:
                pass
            print(f"  Litter Level         : {fmt_pct(robot.litter_level)}  ({fmt(robot.litter_level_state)})")
            if litter_calc is not None:
                print(f"  Litter Level (calc)  : {fmt_pct(litter_calc)}")
            ...
            status_rows.append({
                ...
                "litter_pct_calc": "" if litter_calc is None else round(litter_calc),
...
def build_and_deploy_dashboards(log_dir):
    """Rebuild all three dashboards from the CSV logs and (optionally) deploy them.
    Called after the run's applog row has been written, so the batch dashboard
    includes the run that just finished. Each dashboard is generated independently;
    a failure is noted but never aborts the run."""
    section("\U0001F4C8  DASHBOARDS")
    outputs = []
    try:
        ...
        if info:
            ...
            print(f"  Source  : CSV logs + historical exports")
...
def run() -> None:
    ...
    try:
        ...
        # Write this run's applog row FIRST so the batch dashboard (rebuilt next)
        # includes the run that just finished, then build + deploy the dashboards.
        _append_applog(LOG_DIR, start, status, error)
        if status == "ok":
            try:
                build_and_deploy_dashboards(LOG_DIR)
            except Exception as exc:
                print(f"  Could not build/deploy dashboards: {exc}")
